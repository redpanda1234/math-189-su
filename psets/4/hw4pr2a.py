...
First, please COMMENT OUT any steps other than step 0 in main driver
before you finish the corresponding functions for that step.
...
Note:
1. Please read the instructions and hints carefully, and use the name
of the variables we provided, otherwise, the function may not work.

2. Remember to comment out the TODO comment after you finish each
part.
"""
...
def sigmoid(x):
    ...
    This function applies the sigmoid / logistic function on each
    entry of the input array returns the new array.
...
def grad_logreg(X, y, W, reg=0.0):
    ...
    This function calculates and returns the gradient of W for
    logistic regression.

    HINT:
        1) Recall the log likelihood function for logistic regression
        and get the gradient with respect to the weight matrix, W
...
def NLL(X, y, W, reg=0.0):
    ...
    This function calculates and returns negative log likelihood of
    the logistic regression with l2 regularization

    HINT:
        1) Recall the negative log likelihood function for logistic
        regression.
        2) Use a.sum() to find the summation of all entries in a numpy
        array a
        3) Use np.linalg.norm to find the norm of a given vector
        4) Use np.log to caculate the log of each entry of the input
        array
...
def get_description(X, y, W):
    ...
    HINT:
        ...
        3) Precision = probability of true label being 1 given that
        the predicted label is 1
        4) Recall = probablity of predicted label being 1 given that
        the true label is 1
...
def plot_description(X_train, y_train, X_test, y_test):
    ...
    # TODO: Find the list of accuracy, precision, recall, and f-1
    # score of the prediction given a list of different lambda

    # HINT:
    #     1) First, generate/create a list of different lambda
    #     2) For each lambda, run gradient descent and obtain the
    #     optimal weights
...
if __name__ == '__main__':
    ...
    # =============STEP 0: LOADING DATA=================
    # NOTE: The data is loaded using the code in p2_data.py. Please
    # make sure you read the code in that file and understand how it
    # works.
    ...
    # NOTE: for logistic regression, we only want images with label 0
    # or 1.
    ...
    X_train_logreg = np.hstack((np.ones_like(y_train_logreg),\
                                X_train_logreg))
    X_test_logreg = np.hstack((np.ones_like(y_test_logreg),\
                               X_test_logreg))


    # ========STEP 1a: Gradient descent=========
    # NOTE: Fill in the code in grad_logreg, NLL and grad_descent for
    # this step
    ...
    # NOTE: Fill in the code in newton_step and newton_method for this
    # step
    ...
    # NOTE: You DO NOT need to fill in any additional helper function
    # for this step to run. This step uses what you implemented for
    # the previous two steps to plot.
    print('\n==> Step 2: Generate Convergence Plot...')
