...
def NLL(X, y, W, reg=0.0):
    ...
    HINT:
        ...
        2) Use a.sum() to find the summation of all entries in a numpy
           array a
        3) When perform operations vertically across rows, we use
           axis=0. When perform operations horizontally across
           columns, we use axis=1.
        4) Use np.exp and np.log to calculate the exp and log of each
           entry of the input array

    NOTE: Please use the variable given, NLL.
    """
    gts = (-1 * y * np.log(softmax(X, W))).sum(axis=1).sum()
    NLL = gts + reg * np.diag(W.T @ W).sum()
    return NLL
...
def grad_softmax(X, y, W, reg=0.0):
    ...
    HINT:
        1) Recall the log likelihood function for softmax regression
...
def predict(X, W):
    ...
    This function returns the predicted labels y_pred with dimension m
    x 1

    HINT:
        1) Firstly obtain the probablity matrix according to the
           softmax equation
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
    ...
    This function takes in five arguments:
        ...
        2) y_train_OH, the label of training data with dimension m
           x 1
        3) X_test, the validation data with dimension m x (n + 1)
        4) y_test, the label of validation data with dimension m x 1
        5) lambda_list, a list of different regularization paramters
           that we want to test

    This function generates a plot of accuracy of prediction vs lambda
    and returns the regularization parameter that maximizes the
    accuracy, reg_opt.

    HINT: generate the list of accuracy following the steps below:
        1) Run gradient descent with each parameter to obtain the
           optimal weight
        2) Predicted the label using the weights
        3) Use get_accuracy function provided to calculate the
           accuracy
    """
...
if __name__ == '__main__':
    ...
    lambda_list = [0.01, 0.1, 0.5, 1.0, 10.0, 50.0, 100.0, 200.0,\
                   500.0, 1000.0]
    reg_opt = accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test,
                                 lambda_list)

    print('\n-- Optimal regularization parameter is {:2.2f}'
          .format(reg_opt))
